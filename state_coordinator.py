...
class StateCoordinator:
    ...
    def evaluate(self, evaluated_at=None):
        ...
        with self._lock:
            ...
            candidate = project_village(events, residents, evaluated_at, self._policy,
                                        cursor=cursor, generation=0, capabilities=self._capabilities)
            semantic = dict(candidate)
            semantic.pop("generation", None)
            semantic.pop("evaluated_at", None)
            signature = json.dumps(semantic, sort_keys=True, separators=(",", ":"), allow_nan=False)
    ...
    def delivery(self, generation=None, cursor=None):
        with self._lock:
            ...
            if cursor is not None and self._cursor_namespace(cursor) != self._cursor_namespace(snapshot["cursor"]):
                return {"kind": "reset", "snapshot": copy.deepcopy(snapshot)}
            if generation is not None and generation >= snapshot["generation"]:
                return {"kind": "unchanged", "generation": snapshot["generation"],
    ...
    def wait_for_newer(self, generation, timeout):
        with self._lock:
            self._lock.wait_for(lambda: self._snapshot is not None and
                                self._snapshot["generation"] > generation, timeout=timeout)
            return self._snapshot

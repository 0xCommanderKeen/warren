...
spec = importlib.util.spec_from_file_location(
    "burrow_emit_claude_children", ROOT / "hooks" / "emit.py"
)
...
class ClaudeSubagentAdapterTest(unittest.TestCase):
    ...
    def test_fixture_projects_parent_and_two_children_then_stops_only_one(self):
        ...
        self.assertEqual(
            set(before),
            {
                "claude-code:parent-redacted",
                "claude-code:child-redacted-a",
    ...
    def test_parent_and_two_children_have_independent_identities_and_lineage(self):
        ...
        self.assertEqual(
            emit.adapt_hook("claude", first),
            [
                (
                    "task_started",
                    {
                        "parent_agent_id": "claude-code:parent",
                        "agent_type": "Explore",
                        "prompt": "delegated work",
                    },
                )
    ...
    def test_subagent_stop_ends_only_the_matching_child(self):
        ...
        self.assertEqual(
            emit.adapt_hook("claude", stopped),
            [
                (
                    "session_ended",
                    {

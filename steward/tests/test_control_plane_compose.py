...
checkout is mounted, the API alone may write it, and the daemons link into it rather than
copying out of it.
"""
...
CHECKOUT = "/checkout"
#: The residents tree inside it — the API's ``--residents``.
...
def test_the_api_serves_the_checkout_and_is_the_one_writer(services: dict[str, Any]) -> None:
    ...
    api = services["api"]
    command = api["command"]
    assert command[command.index("--residents") + 1] == RESIDENTS
...
def test_the_daemons_link_into_the_checkout_and_may_not_write_it(
    services: dict[str, Any], name: str
) -> None:
    """Defect three: a copy taken at start is never updated; a link is the live tree."""
    ...
    script = daemon["command"][-1]
    assert "cp -r" not in script, "a copy at start is the tree nothing updates (warren#351)"
    assert f"ln -sfn {RESIDENTS}/pip /sched/residents/pip" in script
    assert f"ln -sfn {CHECKOUT}/steward/skills /sched/skills" in script
    assert "--residents /sched/residents" in script
...
def test_the_daemons_read_one_tree(services: dict[str, Any]) -> None:
    ...
    scheduler = services["scheduler"]["command"][-1]
    watchdog = services["watchdog"]["command"][-1]
    assert scheduler.replace("scheduler run", "watchdog run") == watchdog

...
import { isDefiniteApprovalRefusal } from "../steward/client.js";
...
function ApprovalCard({ item, onSettled }) {
  ...
  async function send(decision, edit) {
    ...
    try {
      ...
      setRefusal(caught);
      // Only contractually pre-decision responses and Steward's exact expiry envelope
      // prove that no operator decision won. Other 4xx responses can be generated by a
      // proxy or arrive after the conditional write, so they remain locked like network
      // and malformed-response failures.
      if (isDefiniteApprovalRefusal(caught)) {

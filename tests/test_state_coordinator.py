...
class StateCoordinatorTests(unittest.TestCase):
    def test_atomically_publishes_new_generations_and_ignores_no_change(self):
        ...
        coordinator = StateCoordinator(lambda: (list(evidence), "cursor-1", 1), lambda: [],
                                       ProjectionPolicy())
    ...
    def test_clock_boundary_publishes_without_new_evidence(self):
        coordinator = StateCoordinator(lambda: ([event("tool_called", tool="Read")], "c", 1),
                                       lambda: [], ProjectionPolicy(stale_seconds=10))
        first = coordinator.evaluate(NOW)
    ...
    def test_reconnect_decision_is_newer_snapshot_or_reset(self):
        ...
        coordinator = StateCoordinator(lambda: ([], current, 4), lambda: [], ProjectionPolicy())
        ...
        self.assertEqual("snapshot", coordinator.delivery(snapshot["generation"] - 1, prior_offset)["kind"])
        self.assertEqual("unchanged", coordinator.delivery(snapshot["generation"], prior_offset)["kind"])
        self.assertEqual("reset", coordinator.delivery(snapshot["generation"], stale_namespace)["kind"])
    ...
    def test_overlapping_evaluations_cannot_publish_an_older_cursor_last(self):
        ...
        def read_events():
            ...
            return [event("tool_called", tool="Read"), event("idle", minutes=1)], "cursor-2", 1
        ...
        newer = threading.Thread(target=lambda: coordinator.evaluate(NOW + dt.timedelta(minutes=1)))
        older.start()

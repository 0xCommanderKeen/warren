...
class CodexEndToEndTest(unittest.TestCase):
    ...
    def test_redacted_fixture_matches_the_documented_hook_shape(self):
        ...
        self.assertTrue({"agent_id", "agent_type"} <= by_name["SubagentStart"].keys())
        self.assertTrue(
            {
                "agent_id",
                "agent_type",
...
"""
    ...
    def test_captured_turn_reaches_the_existing_projection(self):
        ...
        root = [
            event for event in events if event["agent_id"] == "codex:thr_redacted_root"
        ]
        child = [
            event
            for event in events
        ...
        self.assertEqual(
            [event["type"] for event in child], ["task_started", "session_ended"]
        )
        self.assertEqual(
            child[0]["payload"]["parent_agent_id"], "codex:thr_redacted_root"
        )
    ...
    def test_session_end_removes_only_the_root_codex_session(self):
        village = {
            ...
            for villager in self.project(
                self.deliver(self.hooks[:8] + [self.hooks[-1]])
            )
        }
    ...
    def test_long_root_identity_is_preserved_exactly_in_child_lineage(self):
        ...
        root = next(
            event for event in events if event["agent_id"] == "codex:" + session_id
        )
        child = next(event for event in events if event["agent_id"] == "codex:child")
...
class DefensiveCodexInputTest(unittest.TestCase):
    ...
    def test_supported_file_and_notebook_edits_report_artifacts(self):
        for tool, key, path in (
            ...
            self.assertEqual(
                ...
                [("artifact_produced", {"artifact": path})],
                tool,
            )
    ...
    def test_failed_direct_edit_reports_failure_without_claiming_an_artifact(self):
        self.assertEqual(
            ...
            [("tool_failed", {"tool": "NotebookEdit", "error": "cell write failed"})],
        )
    ...
    def test_failed_apply_patch_reports_explicit_failure(self):
        self.assertEqual(
            ...
            [
                (
                    "tool_failed",
                    {"tool": "apply_patch", "error": "Failed to find expected lines"},
                )
            ],
        )
    ...
    def test_ambiguous_apply_patch_response_claims_no_paths(self):
        for response in (
            ...
            if response is not None:
                hook["tool_response"] = response
            expected = (
                ("tool_failed", {"tool": "apply_patch", "error": "write failed"})
    ...
    def test_missing_optional_tool_fields_still_report_only_supported_facts(self):
        self.assertEqual(
            emit.codex_events({"hook_event_name": "PreToolUse"}),
            [
                ("tool_called", {"tool": "?"}),
            ],
        )

...
| **ordering** | atomic decision + outbox row → atomic leased claim → emit → durable acknowledgement → idempotent completion effects (including budget resume) → durable effects marker. The API lifecycle owns one wakeable worker: it drains immediately on startup/new work, retries transient failures with bounded exponential backoff, and wakes at a live lease's exact deadline. A crash after emit and before acknowledgement may repeat the event, so `payload.request_id` is the consumer idempotency key; a crash after acknowledgement resumes from the separate effects marker. Decisions are exactly once; announcements are at least once. |

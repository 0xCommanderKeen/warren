...
| **ordering** | atomic decision + outbox row + correlated API request-log row → atomic leased claim → emit → durable acknowledgement → idempotent completion effects (including budget resume) + request-log completion → durable effects marker. The API lifecycle owns one wakeable worker: it drains immediately on startup/new local work, polls at a bounded interval for commits from other processes, retries transient failures with bounded exponential backoff, and wakes sooner at an exact retry or lease deadline. Shutdown raises if an active pass exceeds its configured deadline. A crash after emit and before acknowledgement may repeat the event, so `payload.request_id` is the consumer idempotency key; a crash after acknowledgement resumes from the separate effects marker. Decisions are exactly once; announcements are at least once. |

...
describe("village archive", () => {
  it("shows real completed tasks and keeps artifacts separate where no task link exists", () => {
    ...
    expect(
      within(
        screen.getByRole("region", { name: "Project not recorded" }),
      ).getByRole("article", { name: "Freeze contract" }),
    ).toBeVisible();
    expect(
      within(screen.getByRole("region", { name: "burrow" })).getByRole(
        "article",
        { name: "report.md" },
      ),
    ).toBeVisible();
    expect(
      screen.queryByRole("article", { name: "Draft the letter" }),
  ...
  it("filters by record type and project, searches names and paths, and sorts newest first", () => {
    openArchive();
    fireEvent.click(
      screen.getByRole("button", { name: "Artifacts", exact: true }),
    );
    expect(
      screen
        .getAllByRole("article")
        .map((record) => within(record).getByRole("heading").textContent),
    ).toEqual(["report.md", "older.md"]);
    fireEvent.change(screen.getByRole("combobox", { name: "Project" }), {
      target: { value: "project:burrow" },
    });
    fireEvent.change(
      screen.getByRole("searchbox", { name: "Search the archive" }),
      { target: { value: "Keeper" } },
    );
    expect(screen.getAllByRole("article")).toHaveLength(2);
    fireEvent.change(screen.getByRole("searchbox"), {
      target: { value: "older.md" },
    });
  ...
  it("shows metadata before explicit preview and copies exact local paths with visible feedback", async () => {
    ...
    expect(record).toHaveTextContent(
      "Preview availability depends on the archive server.",
    );
    expect(
      within(record).queryByRole("link", { name: /Open artifact/ }),
    ).not.toBeInTheDocument();
  ...
  it("only creates external artifact links for HTTP or HTTPS", () => {
    ...
    value.artifacts = [
      "https://example.org/output/report.html",
      "http://example.org/out.txt",
      "javascript:alert(1)",
      "file:///secrets/token",
      "data:text/html,hello",
      "/private/report.md",
    ].map((artifact) => ({ ...value.artifacts[0], artifact }));
    openArchive({ snapshot: value });
    for (const summary of screen.getAllByText("Inspect artifact"))
  ...
  it("opens agent details from actual identities and returns to the village", () => {
    ...
    fireEvent.click(
      within(
        screen.getByRole("article", { name: "Freeze contract" }),
      ).getByRole("button", { name: /Keeper/ }),
    );
  ...
  it("shows an honest empty archive without placeholder accomplishments", () => {
    openArchive({
      snapshot: { ...snapshot(), tasks: [], artifacts: [], journals: [] },
    });
...
it("keeps large archives compact while search still reaches every retained record", () => {
  const data = snapshot();
  data.tasks = [];
  data.journals = [];
  data.artifacts = Array.from({ length: 55 }, (_, index) => ({
    ...data.artifacts[0],
    artifact: `report-${String(index).padStart(2, "0")}.md`,
    ts: new Date(Date.UTC(2026, 8, 5, 12, index)).toISOString(),
  }));
  openArchive({ snapshot: data });
  expect(screen.getAllByRole("article")).toHaveLength(24);
  expect(screen.getByText("24 of 55 matching records shown")).toBeVisible();
  fireEvent.click(screen.getByRole("button", { name: "Show 24 more records" }));
  expect(screen.getAllByRole("article")).toHaveLength(48);
  fireEvent.change(screen.getByRole("searchbox"), {
    target: { value: "report-00.md" },
  });
  expect(screen.getByRole("article")).toHaveAccessibleName("report-00.md");

...
function snapshot() {
  ...
  value.tasks = ["open", "claimed", "failed", "done"].map((state, i) => ({ ...value.tasks[0], id: `task-${i}`, title: `${state} task`, state, assignee: null, claimant: state === "open" ? null : "claude:keeper" }));
  return value;
}
const mount = (extra = {}) => render(<WorkshopBoard snapshot={snapshot()} onSelectAgent={vi.fn()} {...extra} />);

describe("workshop task board", () => {
  it("maps recorded task states to truthful board columns", () => {
    mount();
    for (const [column, state] of [["Queued", "open"], ["Active", "claimed"], ["Needs attention", "failed"], ["Completed", "done"]]) {
      const region = screen.getByRole("region", { name: column, exact: true });
      expect(within(region).getByRole("button")).toHaveTextContent(`${state} task`);
      expect(region).toHaveTextContent(state === "failed" ? "Failed" : state[0].toUpperCase() + state.slice(1));
    }
  ...
  it("opens actual details and only locates an agent on an explicit request", () => {
    ...
    mount({ onSelectAgent });
    fireEvent.click(within(screen.getByRole("region", { name: "Active", exact: true })).getByRole("button"));
    ...
    expect(within(details).getByRole("time")).toHaveAttribute("datetime", fixture.snapshot.tasks[0].updated_at);
    expect(onSelectAgent).not.toHaveBeenCalled();
    fireEvent.click(within(details).getByRole("button", { name: "Locate claimant Keeper" }));
    expect(onSelectAgent).toHaveBeenCalledWith({ kind: "agent", id: "claude:keeper" });
    fireEvent.click(within(details).getByRole("button", { name: "Close task details" }));
    expect(screen.queryByRole("region", { name: "Selected task" })).not.toBeInTheDocument();
  });

  it("searches real task, person and skill fields, then clears filters", () => {
    mount();
    const search = screen.getByRole("searchbox", { name: "Search workshop tasks" });
  ...
  it("preserves unknown agent IDs without offering a broken location action", () => {
    ...
    expect(within(details).queryByRole("button", { name: /Locate/ })).not.toBeInTheDocument();
  });
  ...
  it("updates selected records and removes details when a task leaves the snapshot", () => {
    ...
    expect(screen.getByRole("region", { name: "Selected task" })).toHaveTextContent("Task record · Done");
  ...
  it("folds the board away and restores it without inventing tasks", () => {
    ...
    fireEvent.click(screen.getByRole("button", { name: "Collapse task board" }));
  ...
  it("opens the exact requested task, clears search and unfolds the board once per request", () => {
    const value = snapshot(), onHighlightAgent = vi.fn(), onSelectAgent = vi.fn();
    const view = mount({ snapshot: value, onHighlightAgent, onSelectAgent });
    fireEvent.change(screen.getByRole("searchbox"), { target: { value: "missing" } });
    fireEvent.click(screen.getByRole("button", { name: "Collapse task board" }));
    view.rerender(<WorkshopBoard snapshot={value} taskRequest={{ id: "task-2", nonce: 1 }} onHighlightAgent={onHighlightAgent} onSelectAgent={onSelectAgent} />);
    expect(screen.getByRole("searchbox")).toHaveValue("");
    expect(screen.getByRole("region", { name: "Selected task" })).toHaveTextContent("failed task");
    expect(onHighlightAgent).toHaveBeenLastCalledWith({ kind: "agent", id: "claude:keeper" });
    expect(onSelectAgent).not.toHaveBeenCalled();
    fireEvent.click(screen.getByRole("button", { name: "Close task details" }));
    view.rerender(<WorkshopBoard snapshot={structuredClone(value)} taskRequest={{ id: "task-2", nonce: 1 }} onHighlightAgent={onHighlightAgent} />);
    expect(screen.queryByRole("region", { name: "Selected task" })).not.toBeInTheDocument();
    view.rerender(<WorkshopBoard snapshot={value} taskRequest={{ id: "task-2", nonce: 2 }} onHighlightAgent={onHighlightAgent} />);
    expect(screen.getByRole("region", { name: "Selected task" })).toHaveTextContent("failed task");
  });
  ...
  it("highlights a current claimant or assigned agent without navigating and clears on close", () => {
    ...
    fireEvent.click(within(screen.getByRole("region", { name: "Queued", exact: true })).getByRole("button"));
    expect(onHighlightAgent).toHaveBeenLastCalledWith({ kind: "agent", id: "claude:keeper" });
    fireEvent.click(within(screen.getByRole("region", { name: "Needs attention", exact: true })).getByRole("button"));
    expect(onHighlightAgent).toHaveBeenLastCalledWith(null);
    expect(onSelectAgent).not.toHaveBeenCalled();
    expect(onHighlightAgent.mock.calls.every(([selection]) => selection === null || selection.id === "claude:keeper")).toBe(true);
  });

  it("waits for a requested task to exist without fabricating its details", () => {
    const value = snapshot(), request = { id: "task-2", nonce: 1 };
    const view = mount({ snapshot: { ...value, tasks: [] }, taskRequest: request });
    expect(screen.queryByRole("region", { name: "Selected task" })).not.toBeInTheDocument();

...
describe("personal room stations", () => {
  it("retains stations across arrivals, departures, room changes and reloads", () => {
    ...
    const before = layout.update("lodge:0", [agent("b"), agent("c")]);
    const after = layout.update("lodge:0", [agent("a"), agent("c")]);
    ...
    const returned = restored.update("lodge:0", [agent("b"), agent("c"), agent("a")]);
    expect(returned.stations.map(({ id, position }) => ({ id, position }))).toEqual(after.stations.map(({ id, position }) => ({ id, position })));
    expect(restored.update("lodge:0", []).stations.every(station => station.agent === null)).toBe(true);
  });

  it("expands symmetrically without moving stations and keeps 100 places separate", () => {
    const layout = createRoomLayout();
    const first = layout.update("lodge:0", [agent("z")]);
    const many = layout.update("lodge:0", Array.from({ length: 100 }, (_, i) => agent(String(i))));
    ...
    expect(layout.update("lodge:0", []).width).toBe(many.width);
  });
